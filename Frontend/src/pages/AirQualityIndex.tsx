...
const glassCard = "bg-white/40 backdrop-blur-3xl rounded-[2.5rem] border border-white/60 shadow-[0_20px_50px_rgba(0,0,0,0.05)] overflow-hidden transition-all duration-500 hover:shadow-[0_20px_60px_rgba(59,130,246,0.12)]";
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setZoneDistribution([
          { name: 'West Zone', value: kpiData.aqi - 10, color: '#10B981' },
          { name: 'East Zone', value: kpiData.aqi + 15, color: '#F43F5E' },
          { name: 'North Zone', value: kpiData.aqi - 5, color: '#3B82F6' },
          { name: 'South Zone', value: kpiData.aqi + 8, color: '#F59E0B' },
          { name: 'Central Zone', value: kpiData.aqi + 3, color: '#8B5CF6' }
        ]);
      } catch (error) {
        ...
        setZoneDistribution([
          ...
          { name: 'East Zone', value: 168, color: '#F43F5E' },
          { name: 'North Zone', value: 152, color: '#3B82F6' },
          { name: 'South Zone', value: 158, color: '#F59E0B' },
          { name: 'Central Zone', value: 156, color: '#8B5CF6' }
        ]);
      } finally {
        setLoading(false);
      }
      ...
      <main className="flex-1 ml-64 p-8 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div>
            <div className="flex items-center gap-4 mb-2">
              ...
              <h1 className="text-4xl font-black text-slate-900 tracking-tight">Atmospheric Intelligence</h1>
            </div>
            <p className="text-slate-500 font-bold text-xs uppercase tracking-[0.3em] ml-1">Real-time AQI Monitoring & Forecasting</p>
          </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
             ...
             <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Accessing Sensor Data...</p>
          </div>
        ) : (
          <div className="space-y-8 pb-20">
            ...
            {currentAQI && (
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[
                  { label: 'City Average AQI', val: currentAQI, Icon: Activity, color: 'text-blue-600', trend: 'STABLE' },
                  { label: 'Fine Particulate (PM2.5)', val: (currentAQI * 0.8).toFixed(1), Icon: Wind, color: 'text-emerald-600', trend: 'NORMAL' },
                  { label: 'Inhalable Particles (PM10)', val: (currentAQI * 1.2).toFixed(1), Icon: AlertTriangle, color: 'text-orange-600', trend: 'CAUTION' },
                  { label: 'Monthly Delta', val: '+2.5%', Icon: TrendingUp, color: 'text-indigo-600', trend: 'RISING' }
                ].map((item, i) => {
            ...
            <div className="grid lg:grid-cols-2 gap-8">
              <div className={glassCard + " p-8"}>
                <h3 className="text-xs font-black text-slate-800 uppercase tracking-[0.2em] mb-8 flex items-center gap-2">
                  <Activity size={14} className="text-blue-500" /> Historical Trend
                </h3>
              ...
              <div className={glassCard + " p-8"}>
                <h3 className="text-xs font-black text-slate-800 uppercase tracking-[0.2em] mb-8 flex items-center gap-2">
                  <TrendingUp size={14} className="text-indigo-500" /> AI Forecast Pipeline
                </h3>
                <ResponsiveContainer width="100%" height={300}>
            ...
            <div className="grid lg:grid-cols-2 gap-8">
              <div className={glassCard + " p-8"}>
                <h3 className="text-xs font-black text-slate-800 uppercase tracking-[0.2em] mb-8 flex items-center gap-2">
                  <PieChartIcon size={14} className="text-emerald-500" /> Zonal Health Index
                </h3>
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    <Pie
              ...
              <div className={glassCard + " p-8"}>
                <h3 className="text-xs font-black text-slate-800 uppercase tracking-[0.2em] mb-8 flex items-center gap-2">
                  <BarChart3 size={14} className="text-blue-500" /> Regional Variance
                </h3>
                <ResponsiveContainer width="100%" height={300}>
